...
class Database extends Component {
    ...
    populateDB(tx) {
        ...
        tx.executeSql('CREATE TABLE IF NOT EXISTS Version( '
            + 'version_id INTEGER PRIMARY KEY NOT NULL); ', [], () => { that.successCB() }, (err) => { that.errorCB(err) });

        tx.executeSql('CREATE TABLE IF NOT EXISTS Departments( '
            + 'department_id INTEGER PRIMARY KEY NOT NULL, '
            + 'name VARCHAR(30) ); ', [], () => { that.successCB() }, (err) => { that.errorCB(err) });

        tx.executeSql('CREATE TABLE IF NOT EXISTS Offices( '
            + 'office_id INTEGER PRIMARY KEY NOT NULL, '
            + 'name VARCHAR(20), '
            + 'longtitude FLOAT, '
            + 'latitude FLOAT ) ; ', [], () => { that.successCB() }, (err) => { that.errorCB(err) });
    ...
    queryEmployees(tx) {
        ...
        tx.executeSql('SELECT a.name, b.name as deptName FROM Employees a, Departments b WHERE a.department = b.department_id and a.department=?', [3],
            (tx, results) => { that.queryEmployeesSuccess(tx, results) }, (err) => { that.errorCB(err) });
    }
    ...
    loadAndQueryDB() {
        ...
        db = SQLite.openDatabase(
            ...
            database_size,
            () => { that.openCB() }, (err) => { that.errorCB(err) }
        );
    ...
    deleteDatabase() {
        ...
        this.setState(this.state);
        SQLite.deleteDatabase(database_name, () => { that.deleteCB() }, (err) => { that.errorCB(err) });
    }
